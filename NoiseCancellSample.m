...
%Now, let's load some noise data
filename = 'google.wav';
...
invYd(k) = sum(Sx.*Sw);
e_cont(k)=Yd(k)-invYd(k);   % measure the residue
...
for k=2:T                           % discrete time k
    ...
    Cy=sum(Cx.*Cw);                  % calculate the controller output
    Sx=[Cy Sx(1:length(Sx)-1)];      % propagate to secondary path
    invYd(k)=sum(Sx.*Sw);          % predicted noise
    e_cont(k)=Yd(k)-invYd(k);     % measure the residue
    
    Shx=[Xh(k) Shx(1:15)];           % update the state of Sh(z)
    Xhx=[sum(Shx.*Shw) Xhx(1:15)];   % calculate the filtered x(k)
   % xs(k)=sum(Shx.*Shw);             % for plot/save only
    Cw=Cw+mu*e_cont(k)*Xhx;          % adjust the controller weight
end

% save
%audiowrite('FbLMS-noise.wav',X,fs);
%audiowrite('FbLMS-arriving_noise.wav',Yd,fs);
audiowrite('FbLMS-control.wav',invYd,fs);
%audiowrite('FbLMS-filtered_noise.wav',xs,fs);
audiowrite('FbLMS-output.wav',e_cont,fs);
...
subplot(411)
plot(X);axis('tight')
title(['Input Noise data file: ' filename]);
subplot(412)
plot(Yd);axis('tight')
title('Arriving Noise at sensor position');
subplot(413)
plot([1:T], invYd);axis('tight')
...
title(['Input Noise data file: ' filename]);
subplot(412)
spectrogram(Yd,hamming(512),256,512,fs,'yaxis');
title('Arriving Noise at sensor position');
subplot(413)
spectrogram(invYd,hamming(512),256,512,fs,'yaxis');
title('Predicted Noise');
subplot(414)
spectrogram(e_cont,hamming(512),256,512,fs,'yaxis');
title(' Residue');
...
% zoom in
figure
tt = (length(X)-500+1):length(X);

plot(tt,Yd(tt),'b:.',tt,-invYd(tt),'g:.',tt,e_cont(tt),'r:.');axis('tight')
legend('noise','cocntrol signal','residue')
shg


%comout=[];
%for k=1:T
%    comout(k)=invYd(k)-Yd(k);
%end
%plot([1:T], comout);
plot(X);axis('tight')
title(['Input Noise data file: ' filename]);
